...
export default function DashboardLayout() {
  ...
  const navItems = [
    { name: 'Summary & Routine', path: '/parent/dashboard/summary', icon: <Calendar size={20} /> },
    { name: 'Goals & Career', path: '/parent/dashboard/goals', icon: <Target size={20} /> },
    { name: 'Study & Academics', path: '/parent/dashboard/academics', icon: <BookOpen size={20} /> },
    { name: 'AI Behavioral Profile', path: '/parent/dashboard/behavior', icon: <Brain size={20} /> },
    { name: 'Settings & Alerts', path: '/parent/dashboard/settings', icon: <Settings size={20} /> },
    { name: 'Admin Management', path: '/parent/dashboard/admin', icon: <Shield size={20} /> },
  ];
            ...
            {showNotifications && (
              <div className="absolute top-12 right-0 w-80 glass-panel border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="p-3 text-center bg-white/5">
                  <Link to="/parent/dashboard/settings" onClick={() => setShowNotifications(false)} className="text-xs text-text-muted hover:text-white transition-colors">
